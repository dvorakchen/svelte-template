...
export async function registerUser(phoneNumber: string): Promise<PartOfUser> {
	...
	const attributes = {
		theme: themePrefer.light,
		language: 'zh',
		permissions: [permissions.baseAccess]
	};

	const [user] =
		await sql`INSERT INTO users (username, email, password_hash, phone_number, balance, status, attributes) VALUES
